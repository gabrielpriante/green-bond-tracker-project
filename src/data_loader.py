...
def load_green_bonds(filepath: Optional[str] = None) -> pd.DataFrame:
    """
...
def load_country_geometries(filepath: Optional[str] = None) -> gpd.GeoDataFrame:
    """
    ...
    if 'iso_a3' not in gdf.columns:
        warnings.warn("GeoJSON missing 'iso_a3' column for ISO country codes")
...
def join_bonds_with_geo(bonds_df: pd.DataFrame, 
                        geo_df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    ...
    country_summary = bonds_df.groupby('country_code').agg({
