...
def create_project_type_bar_chart(
    bonds_df: pd.DataFrame, figsize: tuple[int, int] = (12, 6)
) -> plt.Figure:
    """
...
def save_figure(
    fig: plt.Figure, filename: str, output_dir: str | None = None, dpi: int = 300
) -> Path:
    """
...
def create_and_save_all_visuals(
    bonds_df: pd.DataFrame, geo_bonds_df: gpd.GeoDataFrame, output_dir: str | None = None
) -> dict:
    """
    ...
    fig3 = create_choropleth_map(
        geo_bonds_df, column="bond_count", title="Number of Green Bonds by Country"
    )
